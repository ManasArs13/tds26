...
class ControllerCheckoutSimpleCheckoutCart extends SimpleController {
    ...
    // ниже добавил
            if ($this->customer->getGroupId() == 2) {
                if ($product['price_opt'] !== 0) {
				$price = $this->simplecheckout->formatCurrency($this->tax->calculate($product['price_opt'], $product['tax_class_id'], $this->config->get('config_tax')));
                $total = $this->simplecheckout->formatCurrency($this->tax->calculate($product['price_opt'], $product['tax_class_id'], $this->config->get('config_tax')) * $product['quantity']);
                }
